...
3. 下記の仕様に沿ったプログラムを作成してください。

````
Mainクラス　実行クラス
・出力クラスのインスタンスを生成し、実行時パラメータとして受け取った2つのパラメータを渡してコールする

Counterクラス　出力クラス
・返却値のないcountUp(int, int)を持つ公開クラス
・第一引数で指定された数値から、その倍数を第二引数で指定された値まで表示する
・表示形式は「行番号:倍数」とする

※パラメータチェック等のエラー処理は今回は不要です。
余裕があれば取り組んでみてください。
````

4. 上記で作成したプログラムの実行結果をresult.txtというファイルへ出力してください。

`Main 4 50`の実行結果は下記の通りです。
